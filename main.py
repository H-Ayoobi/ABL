...
if __name__ == "__main__":
    ...
    all_TPs_Fast = np.zeros((number_of_iterations, number_of_attempts))
    ...
    for itr in range(number_of_iterations):
        ...
        baf2 = BAF2.BAF2(gen.scenario,gen)
        ...
        for attempt in range(number_of_attempts):
            ...
            guess2 = baf2.generate_second_guess(gen.scenario, saved_scenarios_in_memory_for_other_approaches,
                                              saved_best_recovery_behaviors_in_memory_for_other_approaches,
                                              show_rule=True)
            baf2.update_baf(gen.scenario, gen.best_recovery_behavior, saved_scenarios_in_memory_for_other_approaches,
                           saved_best_recovery_behaviors_in_memory_for_other_approaches)
            if gen.best_recovery_behavior == guess2:
                TP_Fast += 1
            all_TPs_Fast[itr, attempt] = TP_Fast
            ...
            print(f"{attempt}:Our: {TP}, Fast: {TP_Fast}, Decition tree: {DT_TP}")
        ...
        print(f"Our model true positives: {TP_Fast}")
        ...
        print(f"Overal Fast: {np.mean(all_TPs_Fast[:itr+1],axis=0)}")
        print(f"Overal accuracy Fast: {np.mean(all_TPs_Fast[:itr+1],axis=0)/(np.array(range(number_of_attempts)) + 1)}")
